...
const StatRow = ({ label, value, accent = C.blue }) => (
  <div style={{
    ...
    padding: "10px 0", borderBottom: `1px solid ${C.border}`, gap: "16px",
  }}>
...
const AlertRow = ({ text, severity = "warn" }) => {
  ...
  return (
    <div style={{
      ...
      padding: "8px 12px", margin: "6px 0", borderRadius: "4px",
...
// ─── MODULE CARD (original format) ───────────────────────────────────────────
const ModuleCard = ({ title, icon, risk, summary, children, defaultOpen = false }) => {
  const [open, setOpen] = useState(defaultOpen);
  const rc = riskColor(risk || "LOW");
  const rb = riskBgCol(risk || "LOW");
  const rbd = riskBorder(risk || "LOW");

  return (
    <div style={{
      background: C.white, border: `1px solid ${open ? rbd : C.border}`,
      borderLeft: `4px solid ${rc}`, borderRadius: "0 6px 6px 0",
      ...
      >
        ...
        {summary && (
          <span style={{
            ...
            color: C.textSecondary, textAlign: "right", flexShrink: 0, maxWidth: "200px",
        ...
        <span style={{
          ...
          color: rc, background: rb, border: `1px solid ${rbd}`,
          borderRadius: "20px", padding: "3px 12px", flexShrink: 0,
        }}>{risk || "LOW"}</span>
        <span style={{ color: C.textSecondary, fontSize: "13px", flexShrink: 0 }}>
...
const SideCheckRow = ({ id, label, on, color, toggle }) => (
  ...
  >
    <div style={{
      ...
      border: `1.5px solid ${on ? color : C.sidebarFaint}`,
...
// ─── RESULTS VIEW (original format, fully preserved) ─────────────────────────
function ResultsView({ r, riskAssessment, target, onDownload, isDownloading }) {
  ...
  const rb       = riskBgCol(risk);
  const rbd      = riskBorder(risk);
  ...
  return (
    <div style={{ animation: "fadeUp 0.5s ease forwards" }}>
      ...
      {/* Risk Banner */}
      <div style={{
        background: rb, border: `1px solid ${rbd}`,
        borderLeft: `4px solid ${rc}`, borderRadius: "0 8px 8px 0",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          <ModuleCard title="Attack Surface (Subdomains)" icon={<FaSearch />} risk={r.securityTrails.risk || "LOW"} summary={`${r.securityTrails.subdomainCount} subdomains`}>
            ...
            {(r.securityTrails.subdomains || []).length > 0 && (
              <div style={{ marginTop: "12px" }}>
                ...
                {r.securityTrails.subdomains.length > 12 && (
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: "13px", color: C.textMuted, marginTop: "6px", fontWeight: 500 }}>+{r.securityTrails.subdomains.length - 12} more</div>
                )}
              </div>
            )}
          </ModuleCard>
        ...
        {r.vulnerabilities && Object.entries(r.vulnerabilities).map(([key, vuln]) => {
          ...
          if (key === "xss") {
            ...
            return (
              <ModuleCard key={key} title="XSS (Cross-Site Scripting)" icon={<FaBug />} risk={found ? "HIGH" : "LOW"} summary={found ? `Vulnerable — ${endpoints.length} reflected${dom.found ? " + DOM" : ""}${stored.found ? " + Stored" : ""}` : "Not detected"} defaultOpen={found}>
          ...
          return (
            <ModuleCard key={key} title={labels[key] || key} icon={<FaBug />} risk={found ? "HIGH" : "LOW"} summary={found ? "Vulnerable" : "Not detected"} defaultOpen={found}>
              <StatRow label="STATUS" value={found ? "Vulnerable" : "Clean"} accent={found ? C.red : C.green} />
      ...
      <div style={{ marginTop: "36px", textAlign: "center", paddingBottom: "40px" }}>
        <button
          ...
          style={{
            ...
            color: C.white, background: isDownloading ? C.accentBorder : C.accent,
            border: "none", padding: "14px 32px", borderRadius: "4px",
            cursor: isDownloading ? "not-allowed" : "pointer",
            display: "inline-flex", alignItems: "center", gap: "10px",
            boxShadow: `0 4px 16px ${C.accent}40`, transition: "all 0.2s",
          }}
  ...
  return (
    <div style={{ background: C.bg, height: "100vh", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      ...
      <header style={{
        ...
        padding: "0 28px", background: C.sidebarBg,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px", cursor: "pointer" }} onClick={() => window.location.href = "/"}>
          <svg viewBox="0 0 36 36" width="24" height="24">
            ...
            <circle cx="18" cy="18" r="3" fill={C.purple}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div style={{
            ...
            background: scanning ? "rgba(109,40,217,0.2)" : "rgba(255,255,255,0.06)",
            border: `1px solid ${scanning ? C.purple + "60" : C.sidebarBorder}`,
            transition: "all 0.3s",
          }}>
            <div style={{
              width: "7px", height: "7px", borderRadius: "50%",
              background: scanning ? C.purple : C.sidebarFaint,
              animation: scanning ? "pulse 1.4s ease-in-out infinite" : "none",
              boxShadow: scanning ? `0 0 8px ${C.purple}` : "none",
              transition: "all 0.3s",
            }} />
            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "10px", color: scanning ? C.accentBorder : C.sidebarMuted }}>
              {scanning ? `Scanning ${fmtTime(elapsed)}` : "Ready"}
            </span>
          </div>
        </div>
      </header>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", height: "calc(100vh - 52px)" }}>
        ...
        <aside style={{
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "18px 10px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 6px", marginBottom: "14px" }}>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "9px", color: C.sidebarFaint, letterSpacing: "0.12em" }}>
                MODULES ({selected.size}/{ALL_MODULE_IDS.length})
              </span>
              ...
              return (
                <div key={group.label} style={{ marginBottom: "18px" }}>
                  <div
                    ...
                    style={{ display: "flex", alignItems: "center", gap: "8px", padding: "4px 8px", cursor: "pointer", marginBottom: "4px", borderRadius: "4px" }}
                    onMouseEnter={e => e.currentTarget.style.background = C.sidebarHover}
                    onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                  >
                    ...
                    <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "11px", color: allOn ? C.sidebarText : C.sidebarMuted, flex: 1, letterSpacing: "0.04em", transition: "color 0.2s" }}>
                      {group.label}
                    </span>
                    <span style={{
                      fontFamily: "'DM Mono', monospace", fontSize: "9px",
                      color: allOn ? group.color : C.sidebarFaint,
                      background: allOn ? group.color + "18" : "transparent",
                      border: `1px solid ${allOn ? group.color + "40" : C.sidebarBorder}`,
                      padding: "1px 7px", borderRadius: "10px", transition: "all 0.2s",
                    }}>{gSel}/{group.modules.length}</span>
                  </div>
        ...
        <main style={{ flex: 1, overflowY: "auto", background: C.bg, display: "flex", flexDirection: "column", height: "100%" }}>
          ...
          {/* Sticky re-scan bar — visible whenever results exist OR scanning */}
          {(results || scanning) && (
            <div style={{
              ...
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: "10px", color: C.textXMuted, letterSpacing: "0.1em", flexShrink: 0, whiteSpace: "nowrap" }}>
                NEW TARGET
              </div>
              <input
                value={input}
              ...
              <button
                ...
                style={{
                  ...
                  color: (scanning || !input.trim() || selected.size === 0) ? C.textXMuted : C.white,
                  background: (scanning || !input.trim() || selected.size === 0) ? C.border : C.accent,
                  border: "none",
                  padding: "9px 24px",
                  borderRadius: "4px",
                  cursor: (scanning || !input.trim() || selected.size === 0) ? "not-allowed" : "pointer",
                  display: "flex", alignItems: "center", gap: "8px",
                  flexShrink: 0,
                  boxShadow: (scanning || !input.trim() || selected.size === 0) ? "none" : `0 2px 8px ${C.accent}50`,
              ...
              >
                {scanning
                  ? <span style={{ animation: "spin 0.8s linear infinite", display: "inline-block", fontSize: "11px" }}>◌</span>
          ...
          {/* No results yet — welcome state */}
          {!results && !scanning && (
            <div style={{ padding: "52px 48px", animation: "fadeUp 0.5s ease both", maxWidth: "780px" }}>

              {/* Breadcrumb with left accent bar — exactly like the image */}
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "22px" }}>
                ...
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "11px", color: C.accent, letterSpacing: "0.14em" }}>
                  // MODULE_03 / CUSTOM_SCAN
                </span>
              </div>

              {/* Big bold uppercase heading — Rajdhani like the image */}
              <h1 style={{
                fontFamily: "'Rajdhani', sans-serif",
                fontWeight: 700,
              ...
              {/* Accent underline bar — like the image */}
              <div style={{ width: "48px", height: "3px", background: C.accent, borderRadius: "1px", marginBottom: "44px" }} />

              {/* Input card — styled exactly like the image: white card, top accent border, monospace label */}
              <div style={{
                background: C.white,
                border: `1px solid ${C.border}`,
                borderTop: `3px solid ${C.accent}`,
                ...
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: (scanning || !input.trim() || selected.size === 0) ? "none" : `0 2px 10px ${C.accent}50`,
                      transition: "all 0.15s",
                    }}
                    onMouseEnter={e => { if (!scanning && input.trim() && selected.size > 0) { e.currentTarget.style.background = C.accentHover; e.currentTarget.style.transform = "translateY(-1px)"; }}}
                    onMouseLeave={e => { e.currentTarget.style.background = (scanning || !input.trim() || selected.size === 0) ? C.border : C.accent; e.currentTarget.style.transform = "translateY(0)"; }}
                  >
                    {scanning
                      ? <span style={{ animation: "spin 0.8s linear infinite", display: "inline-block", fontSize: "12px" }}>◌</span>
              ...
              <div style={{ display: "flex", gap: "16px", marginTop: "32px", flexWrap: "wrap" }}>
                {[
                  { label: "Total Modules", value: ALL_MODULE_IDS.length },
                  { label: "Selected", value: selected.size },
                  { label: "Categories", value: MODULE_GROUPS.length },
                ].map(stat => (
          ...
          {scanning && (
            <div style={{ padding: "52px 48px", animation: "fadeUp 0.3s ease both", maxWidth: "780px" }}>
              ...
              <p style={{ fontFamily: "'DM Mono', monospace", fontSize: "11px", color: C.textMuted, marginBottom: "24px", letterSpacing: "0.04em" }}>
                Vulnerability modules may take up to 3 minutes. Please wait.
              </p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                {[...selected].map(id => {
                  ...
                  return (
                    <span key={id} style={{
                      ...
                      color: grp?.color || C.purple,
                      background: (grp?.color || C.purple) + "12",
                      border: `1px solid ${(grp?.color || C.purple)}30`,
